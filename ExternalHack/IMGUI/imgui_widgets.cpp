...
bool ImGui::InputScalarAsWidgetReplacement(const ImRect& bb, ImGuiID id, const char* label, ImGuiDataType data_type, void* data_ptr, const char* format)
{
	...
	bool value_changed = InputTextEx(label, data_buf, IM_ARRAYSIZE(&data_buf), bb.GetSize().x, ImVec2(0,0), flags, 0, 0);
	if (g.ScalarAsInputTextId == 0)
	{
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power)
{
	...
	if (hovered)
	{
		...
		//RenderTextOverlay(ImVec2(grab_bb.Min.x, grab_bb.Min.y + ((grab_bb.Max.y - grab_bb.Min.y) / 2.f)) + ImVec2(-3, 10), value_buf[0], value_buf_end[0]);
		ImGui::PopStyleColor();
	}
	float centreX = (frame_bb.Max.x - frame_bb.Min.x )/2.f;
	float centreY = (frame_bb.Max.y - frame_bb.Min.y )/2.f;

//	window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(255, 0, 0,255), 0, 15);

	
	RenderFrame(ImVec2(frame_bb.Min.x, frame_bb.Min.y + centreY - 2), ImVec2(grab_bb.Min.x, frame_bb.Max.y - ((frame_bb.Max.y - frame_bb.Min.y) / 2.5f) + 4), ImColor(15, 15,15, 200), true, 5.f);
	RenderFrame(ImVec2(frame_bb.Min.x, frame_bb.Min.y + centreY + 3) - ImVec2(0, 3), ImVec2(grab_bb.Min.x, frame_bb.Max.y - ((frame_bb.Max.y - frame_bb.Min.y) / 2.5f)) + ImVec2(0, 3), ImColor(155, 10, 10, 255), true, 5.f);
	window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x, grab_bb.Min.y + ((grab_bb.Max.y - grab_bb.Min.y) / 2.f) + 2.5), 7.f, ImColor(190, 10, 10, 255), 32);
	RenderText(ImVec2(grab_bb.Min.x, grab_bb.Min.y + ((grab_bb.Max.y - grab_bb.Min.y) / 2.f)) + ImVec2(-3, 10), value_buf[0], value_buf_end[0]);

	// Display value using user-provided display format so user can add prefix/suffix/decorations to the value. 
	if (label_size.x > 0.0f)
		RenderText(ImVec2(frame_bb.Max.x - grab_bb.Max.x + grab_bb.Min.x + style.ItemInnerSpacing.x, frame_bb.Min.y + style.FramePadding.y + 3.8), label);
